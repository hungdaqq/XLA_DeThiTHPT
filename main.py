"""
test_grid_intersection_v2.py – Phân tích grid mà không cần bubble detection
"""
...
# ============================================================
# 3. CIRCLE DETECTION AND FILL RATIO CALCULATION
# ============================================================

def detect_circle_in_box(warped: np.ndarray, x1: int, y1: int, x2: int, y2: int, fill_threshold: float = 0.6, use_otsu: bool = False) -> tuple:
    """Detect and analyze circle/bubble inside a box region
    Returns: (is_filled, fill_ratio, circle_info)
    """
    ...
    # Apply Gaussian blur with stronger blur
    blurred = cv2.GaussianBlur(roi, (7, 7), 1.5)
    
    # Threshold for dark pixels (ink/pen)
    if use_otsu:
    ...
    # Calculate basic fill ratio first
    dark_pixels = np.sum(binary > 0)
    total_pixels = roi.size
    fill_ratio = dark_pixels / max(total_pixels, 1)
    ...
    circles = cv2.HoughCircles(
        ...
        cv2.HOUGH_GRADIENT,
        dp=1,
        minDist=max(box_w, box_h) * 0.8,
        param1=30,
        param2=10,
        minRadius=max(1, int(min(box_w, box_h) * 0.15)),
        maxRadius=max(10, int(min(box_w, box_h) * 0.8))
    )
    
    circle_info = None
    
    # If circle detected, use it for refined fill detection
    if circles is not None and len(circles[0]) > 0:
        # Find circle closest to box center
        circles = circles[0]
        best_circle = None
        min_dist = float('inf')
        
        for circle in circles:
            cx_c, cy_c, radius = circle
            dist = np.sqrt((cx_c - box_cx)**2 + (cy_c - box_cy)**2)
            if dist < min_dist:
                min_dist = dist
                best_circle = circle
        
        if best_circle is not None:
            cx_c, cy_c, radius = best_circle
            
            # Create mask for the detected circle
            mask = np.zeros_like(binary)
            cv2.circle(mask, (int(cx_c), int(cy_c)), int(radius), 255, -1)
            
            # Calculate fill ratio within circle
            circle_region = binary[mask > 0]
            if circle_region.size > 0:
                filled_pixels = np.sum(circle_region > 0)
                fill_ratio = filled_pixels / max(circle_region.size, 1)
                circle_info = (int(cx_c + x1), int(cy_c + y1), int(radius))
    
    # Determine if filled based on threshold
    is_filled = fill_ratio > fill_threshold
    
    return is_filled, fill_ratio, circle_info


def analyze_box(warped: np.ndarray, x1: int, y1: int, x2: int, y2: int, fill_threshold: float = 0.6, use_otsu: bool = False) -> dict:
    """Analyze a single grid box using circle detection"""
    is_filled, fill_ratio, circle_info = detect_circle_in_box(warped, x1, y1, x2, y2, fill_threshold, use_otsu)
    ...
    return {
        ...
        "has_circle": circle_info is not None
    }


# ============================================================
# HELPER FUNCTIONS FOR SBD/MADE DETECTION
# ============================================================
...
def analyze_grid(warped: np.ndarray) -> Dict:
    """Phân tích grid tại mỗi giao điểm"""
    gray = cv2.cvtColor(warped, cv2.COLOR_BGR2GRAY)
    H, W = warped.shape[:2]
    results = {}
    ...
    for grp_idx, grp in enumerate(cfg["groups"]):
        ...
        for col in range(n_cols):
            ...
            for row in range(n_rows):
                ...
                sobaodanh_grid.append({
                    "digit": row_label, "col": col,
                    "box_bounds": (0, 0, 0, 0),  # Not used in new approach
    ...
    results["sobaodanh"] = sobaodanh_grid
    ...
    for group_idx, grp in enumerate(cfg["groups"]):
        ...
        for row in range(n_rows):
            ...
            for col in range(n_cols):
                ...
                box_data = analyze_box(warped, col_x1, row_y1, col_x2, row_y2, fill_thresh)
                ...
                phan1_grid.append({
                    ...
                    "filled": filled, "fill_ratio": box_data["fill_ratio"],
                    "circle_info": box_data["circle_info"]
                })
                
                status = "✓" if filled else "·"
                print(f"  Q{q_num:02d}-{choice}: {status} ({box_data['fill_ratio']:.2f})")
    ...
    for group_idx, grp in enumerate(cfg["groups"]):
        ...
        for row in range(n_rows):
            ...
            for col in range(2):
                ...
                has_circle = box_data["has_circle"]
                circle_status = "[CIRCLE]" if has_circle else "[NO_CIRCLE]"
                ...
                print(f"  Cau{cau_odd}-{cfg['rows'][row]}-{col_label}: {filled_status} {circle_status} ({box_data['fill_ratio']:.2f})")
            ...
            for col in range(2):
                ...
                print(f"  Cau{cau_even}-{cfg['rows'][row]}-{col_label}: {filled_status} {circle_status} ({box_data['fill_ratio']:.2f})")
    ...
    for group_idx, grp in enumerate(cfg["groups"]):
        ...
        for row in range(n_rows):
            ...
            for col in col_range:
                ...
                if col == col_range[0]:
                    filled_status = "✓" if filled else "·"
                    print(f"  Cau{cau_num}-{row_label}: {filled_status} {circle_status} ({box_data['fill_ratio']:.2f})")
...
def visualize(warped: np.ndarray, grid_data: Dict, out_path: str):
    """Vẽ group boxes + grid lines + circles xanh (opacity 50%) cho những ô được fill"""
    out = warped.copy()
    H, W = warped.shape[:2]
    overlay = out.copy()
    gray = cv2.cvtColor(warped, cv2.COLOR_BGR2GRAY)
    ...
    RADIUS = 8
    GREEN = (0, 255, 0)
    GRID_COLOR = (0, 255, 0)  # Xanh cho grid lines (như phan1, 2, 3)
    BOX_COLOR = (100, 200, 0)     # Xanh nhạt cho group boxes
    ...
    for grp_idx, grp in enumerate(cfg["groups"]):
        ...
        # NO grid lines - only render circles
    ...
    for section in ["sobaodanh", "made", "phan1", "phan2", "phan3"]:
        for item in grid_data[section]:
            if item["filled"]:
                if item["circle_info"] is not None:
                    # Draw detected filled circle
                    cx, cy, radius = item["circle_info"]
                    cv2.circle(overlay, (cx, cy), radius, GREEN, -1)
                else:
                    # Fallback: draw box if no circle detected
    ...
    for section in ["sobaodanh", "made", "phan1", "phan2", "phan3"]:
        for item in grid_data[section]:
            if item["filled"] and "box_bounds" in item:
                x1, y1, x2, y2 = item["box_bounds"]
    ...
    for section in ["sobaodanh", "made", "phan1", "phan2", "phan3"]:
        for item in grid_data[section]:
            if not item["filled"] and "box_bounds" in item:
                x1, y1, x2, y2 = item["box_bounds"]
...
def process(image_path: str, out_dir: str = "./outputs_grid", debug: bool = True):
    ...
    print(f"\n{'='*60}")
    print(f"  GRID ANALYSIS (No Bubble Detection)")
    print(f"  {image_path}")
    print(f"{'='*60}")
